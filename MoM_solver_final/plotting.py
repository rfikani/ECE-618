...
def plot_z_matrix_structure(straight_results, bent_results):
    straight_item = sorted(straight_results, key=lambda x: x["Ns"])[-1]
    bent_item = sorted(bent_results, key=lambda x: x["frequency_MHz"])[0]

    z_straight = np.log10(np.abs(straight_item["zmat"]) + 1.0e-16)
    z_bent = np.log10(np.abs(bent_item["zmat"]) + 1.0e-16)

    combined = np.concatenate([z_straight.ravel(), z_bent.ravel()])
    vmin = np.percentile(combined, 1.0)
    vmax = np.percentile(combined, 99.5)

    cmap = matrix_colormap()

    fig1, ax1 = plt.subplots(figsize=(3.45, 2.75))
    ...
    ax1.set_title(r"Straight dipole")
    ax1.set_xlabel(r"Source index $n$")
    ax1.set_ylabel(r"Test index $m$")
    ax1.grid(False)
    ...
    cbar1.set_label(r"$\log_{10}|Z_{mn}|$", labelpad=6)
    cbar1.ax.tick_params(direction="in", width=0.75, length=3.0)

    fig1.subplots_adjust(left=0.16, right=0.88, bottom=0.17, top=0.90)

    fig2, ax2 = plt.subplots(figsize=(3.45, 2.75))
    ...
    ax2.set_title(r"Ninety-degree bent wire")
    ax2.set_xlabel(r"Source index $n$")
    ax2.set_ylabel(r"Test index $m$")
    ax2.grid(False)
    ...
    cbar2.set_label(r"$\log_{10}|Z_{mn}|$", labelpad=6)
    cbar2.ax.tick_params(direction="in", width=0.75, length=3.0)

    fig2.subplots_adjust(left=0.16, right=0.88, bottom=0.17, top=0.90)
